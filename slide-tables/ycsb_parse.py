    ...
    for path in glob.glob(args.glob):
        ...
        with open(path, "r", errors="ignore") as f:
            for line in f:
                ...
                if m2:
                    op = m2.group(1); name = m2.group(2); val_us = float(m2.group(3))
                    d = metrics.setdefault(op, {"avg_ms":"", "p95_ms":"", "p99_ms":"", "max_ms":""})
                    if name.startswith("AverageLatency"): d["avg_ms"] = f"{val_us/1000:.2f}"
                    elif name.startswith("95th"): d["p95_ms"] = f"{val_us/1000:.2f}"
                    elif name.startswith("99th"): d["p99_ms"] = f"{val_us/1000:.2f}"
                    elif name.startswith("MaxLatency"): d["max_ms"] = f"{val_us/1000:.2f}"
        system, phase, workload = infer_from_name(path)
        if args.system: system = args.system
        if args.workload: workload = args.workload
        row = {"file": os.path.basename(path), "system": system, "phase": phase, "workload": workload,
               "threads": args.threads, "target_ops": args.target, "recordcount": args.recordcount,
               "maxexecutiontime_sec": args.maxexecutiontime, "throughput_ops_sec": overall or ""}
        ...
        rows.append(row)
    if not rows:
        print("No input matched or no metrics parsed.", file=sys.stderr); sys.exit(2)
